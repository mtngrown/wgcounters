...
RSpec.describe WGCounters::FirstCav do
  describe '#to_svg' do
    ...
    it 'includes the dark green outer shield' do
      paths = doc.css('path')
      fills = paths.map { |p| p['fill'] }
      expect(fills).to include('#2c4f42')
    end

    it 'includes the gold inner shield' do
      paths = doc.css('path')
      fills = paths.map { |p| p['fill'] }
      expect(fills).to include('#ffc200')
    end

    it 'applies insignia transform' do
      groups = doc.css('g[transform]')
      transforms = groups.map { |g| g['transform'] }
      expect(transforms).to include('translate(312,255) scale(2.0)')
    end
  end
