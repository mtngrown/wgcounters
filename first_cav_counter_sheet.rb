...
require 'nokogiri'
require_relative 'first_cav'

# 8 rows, 10 columns = 80 1st Cavalry counters

def row(xml, row_number)
  (1..10).each do |column|
    xml.g(transform: "translate(#{(column - 1) * 200},#{row_number * 200}) scale(0.2)") do
      FirstCav.new.build_counter(xml)
    end
  end
end
...
File.write('generated/first-cav-counter-sheet.svg', to_svg)
